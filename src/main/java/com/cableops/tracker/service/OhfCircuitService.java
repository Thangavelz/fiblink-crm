...
public class OhfCircuitService {
    ...
    private static final List<String> ALLOWED_CIRCUIT_TYPES =
            List.of("Infra", "OHF", "UG", "MW", "Ent Customer");

    private static final List<String> ALLOWED_STATUSES =
            List.of("Under Provisioning", "Active", "Pending/Hold", "Suspended", "Acceptance Pending");

    private static final List<String> ALLOWED_FIBER_TYPES =
            List.of("Single Core Single Path", "Single Core Dual Path",
    ...
    private void validate(OhfCircuitRequest req) {
        ...
        // ── Suspended requires both date and reason ───────────────────────────
        if ("Suspended".equals(req.getCircuitStatus())) {
            if (req.getSuspendedDate() == null)
                throw new RuntimeException("A suspension date is required when status is Suspended.");
            if (req.getSuspendedReason() == null || req.getSuspendedReason().isBlank())
                throw new RuntimeException("A suspension reason is required when status is Suspended.");
        }
    }
    ...
    private void map(OhfCircuit c, OhfCircuitRequest r) {
        ...
        // ── Suspension fields — set when Suspended, clear otherwise ───────────
        if ("Suspended".equals(r.getCircuitStatus())) {
            if (r.getSuspendedDate()   != null) c.setSuspendedDate(r.getSuspendedDate());
            if (r.getSuspendedReason() != null) c.setSuspendedReason(r.getSuspendedReason());
        } else if (r.getCircuitStatus() != null) {
            // Status explicitly changed away from Suspended — clear the fields
            c.setSuspendedDate(null);
            c.setSuspendedReason(null);
        }
        ...
        if (r.getAccountId() != null) {
            ...
            if (r.getAccountName() != null) {
                ...
                accountRepo.findById(r.getAccountId())
                        .ifPresent(acc -> c.setAccountName(acc.getName()));
            }
        }
    ...
    private OhfCircuitResponse toResponse(OhfCircuit c, OhfCircuitRequest req) {
        ...
        // ── Suspension fields — always read from entity ───────────────────────
        res.setSuspendedDate(c.getSuspendedDate());
        res.setSuspendedReason(c.getSuspendedReason());

        // ── Teams / assigned users — only available when req is present ───────
        if (req != null) {
