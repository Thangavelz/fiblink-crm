...
public class OhfCircuitService {
	...
	// ── Allowed enums ─────────────────────────────────────────────────────────
	private static final List<String> ALLOWED_CIRCUIT_TYPES = List.of("Infra", "OHF", "UG", "MW", "Ent Customer");

	private static final List<String> ALLOWED_STATUSES = List.of("Under Provisioning", "Active", "Pending/Hold",
			"Suspended", "Acceptance Pending");

	private static final List<String> ALLOWED_FIBER_TYPES = List.of("Single Core Single Path", "Single Core Dual Path",
			"Dual Core Single Path", "Dual Core Dual Path");
	...
	public OhfCircuitResponse get(String id) {
		OhfCircuit c = repo.findById(id).orElseThrow(() -> new RuntimeException("Circuit not found: " + id));
	...
	public List<OhfCircuitResponse> listByAccount(String accountId) {
		return repo.findByAccountId(accountId).stream().map(c -> toResponse(c, null)).toList();
	}
	...
	private void validate(OhfCircuitRequest req) {
		if (req.getCircuitType() != null && !ALLOWED_CIRCUIT_TYPES.contains(req.getCircuitType())) {
			throw new RuntimeException(
					"Invalid circuitType '" + req.getCircuitType() + "'. Allowed: " + ALLOWED_CIRCUIT_TYPES);
		}
		if (req.getCircuitStatus() != null && !ALLOWED_STATUSES.contains(req.getCircuitStatus())) {
			throw new RuntimeException(
					"Invalid circuitStatus '" + req.getCircuitStatus() + "'. Allowed: " + ALLOWED_STATUSES);
		}
		if (req.getFiberType() != null && !ALLOWED_FIBER_TYPES.contains(req.getFiberType())) {
			throw new RuntimeException(
					"Invalid fiberType '" + req.getFiberType() + "'. Allowed: " + ALLOWED_FIBER_TYPES);
		}
	}
	...
	private void map(OhfCircuit c, OhfCircuitRequest r) {
		...
		if (r.getName() != null)
			c.setName(r.getName());
		if (r.getCircuitType() != null)
			c.setCircuitType(r.getCircuitType());
		if (r.getCircuitStatus() != null)
			c.setCircuitStatus(r.getCircuitStatus());
		if (r.getFiberType() != null)
			c.setFiberType(r.getFiberType());
		if (r.getActualLinkDistance() != null)
			c.setActualLinkDistance(r.getActualLinkDistance());
		...
		if (r.getAccountId() != null) {
			...
			if (r.getAccountName() != null) {
				...
				accountRepo.findById(r.getAccountId()).ifPresent(acc -> c.setAccountName(acc.getName()));
			}
		}

		// ── Contact ───────────────────────────────────────────────────────────
		if (r.getContactId() != null)
			c.setContactId(r.getContactId());
		if (r.getContactName() != null)
			c.setContactName(r.getContactName());

		// ── Address blocks — use the SAVED circuit type as fallback ───────────
		// This prevents overwriting address fields when circuitType is null in the
		// request
		...
		if (isTowerType) {
			if (r.getTowerAAddressStreet() != null)
				c.setTowerAAddressStreet(r.getTowerAAddressStreet());
			if (r.getTowerAAddressCity() != null)
				c.setTowerAAddressCity(r.getTowerAAddressCity());
			if (r.getTowerAAddressState() != null)
				c.setTowerAAddressState(r.getTowerAAddressState());
			if (r.getTowerAAddressCountry() != null)
				c.setTowerAAddressCountry(r.getTowerAAddressCountry());
			if (r.getTowerAAddressPostalCode() != null)
				c.setTowerAAddressPostalCode(r.getTowerAAddressPostalCode());
			if (r.getTowerALocation() != null)
				c.setTowerALocation(r.getTowerALocation());
			if (r.getTowerALatLong() != null)
				c.setTowerALatLong(r.getTowerALatLong());

			if (r.getTowerBAddressStreet() != null)
				c.setTowerBAddressStreet(r.getTowerBAddressStreet());
			if (r.getTowerBAddressCity() != null)
				c.setTowerBAddressCity(r.getTowerBAddressCity());
			if (r.getTowerBAddressState() != null)
				c.setTowerBAddressState(r.getTowerBAddressState());
			if (r.getTowerBAddressCountry() != null)
				c.setTowerBAddressCountry(r.getTowerBAddressCountry());
			if (r.getTowerBAddressPostalCode() != null)
				c.setTowerBAddressPostalCode(r.getTowerBAddressPostalCode());
			if (r.getTowerBLocation() != null)
				c.setTowerBLocation(r.getTowerBLocation());
			if (r.getTowerBLatLong() != null)
				c.setTowerBLatLong(r.getTowerBLatLong());
		} else {
			if (r.getCustomerAddressStreet() != null)
				c.setCustomerAddressStreet(r.getCustomerAddressStreet());
			if (r.getCustomerAddressCity() != null)
				c.setCustomerAddressCity(r.getCustomerAddressCity());
			if (r.getCustomerAddressState() != null)
				c.setCustomerAddressState(r.getCustomerAddressState());
			if (r.getCustomerAddressCountry() != null)
				c.setCustomerAddressCountry(r.getCustomerAddressCountry());
			if (r.getCustomerAddressPostalCode() != null)
				c.setCustomerAddressPostalCode(r.getCustomerAddressPostalCode());
			if (r.getCustomerlocation() != null)
				c.setCustomerlocation(r.getCustomerlocation());
			if (r.getCustometLatLong() != null)
				c.setCustometLatLong(r.getCustometLatLong());

			if (r.getPOPAddressStreet() != null)
				c.setPOPAddressStreet(r.getPOPAddressStreet());
			if (r.getPOPAddressCity() != null)
				c.setPOPAddressCity(r.getPOPAddressCity());
			if (r.getPOPAddressState() != null)
				c.setPOPAddressState(r.getPOPAddressState());
			if (r.getPOPAddressCountry() != null)
				c.setPOPAddressCountry(r.getPOPAddressCountry());
			if (r.getPOPAddressPostalCode() != null)
				c.setPOPAddressPostalCode(r.getPOPAddressPostalCode());
			if (r.getPOPLocation() != null)
				c.setPOPLocation(r.getPOPLocation());
			if (r.getPOPLatLong() != null)
				c.setPOPLatLong(r.getPOPLatLong());
		}

		// ── Commercial (all types) ────────────────────────────────────────────
		if (r.getPONumber() != null)
			c.setPONumber(r.getPONumber());
		if (r.getPODate() != null)
			c.setPODate(r.getPODate());
		if (r.getPodistanceinkm() != null)
			c.setPodistanceinkm(r.getPodistanceinkm());
		if (r.getAcceptanceDate() != null)
			c.setAcceptanceDate(r.getAcceptanceDate());
		if (r.getOneTimeCharges() != null)
			c.setOneTimeCharges(r.getOneTimeCharges());
		if (r.getOneTimeChargesCurrency() != null)
			c.setOneTimeChargesCurrency(r.getOneTimeChargesCurrency());
		if (r.getArcAmount() != null)
			c.setArcAmount(r.getArcAmount());
		if (r.getArcasperRatecardperKm() != null)
			c.setArcasperRatecardperKm(r.getArcasperRatecardperKm());
		if (r.getArcasperRatecardperKmCurrency() != null)
			c.setArcasperRatecardperKmCurrency(r.getArcasperRatecardperKmCurrency());
		if (r.getAmountPerKmPerMonth() != null)
			c.setAmountPerKmPerMonth(r.getAmountPerKmPerMonth());
	}
